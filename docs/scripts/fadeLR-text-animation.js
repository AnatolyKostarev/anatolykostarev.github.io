function initFadeLRGenerator() {
  const container = document.getElementById('fadeLR-generator')
  if (!container) return

  const animationIdInput = container.querySelector('#fadeLR-animationID')
  const delayBeforeStartInput = container.querySelector(
    '#fadeLR-delayBeforeStart'
  )
  const animationDelayInput = container.querySelector('#fadeLR-animationDalay')
  const speedInput = container.querySelector('#fadeLR-animationSpeed')
  const slowdownEffectCheckbox = container.querySelector(
    '#fadeLR-slowdownEffect'
  )
  const endSlowdownEffectCheckbox = container.querySelector(
    '#fadeLR-endSlowdownEffect'
  )
  const generateBtn = container.querySelector('#generate-fadeLR')
  const copyBtn = container.querySelector('#copy-fadeLR')
  const title = container.querySelector('#title')
  const output = container.querySelector('#fadeLR-output')
