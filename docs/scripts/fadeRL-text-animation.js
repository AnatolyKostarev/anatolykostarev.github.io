function initFadeRLGenerator() {
  const container = document.getElementById('fadeRL-generator')
  if (!container) return

  const animationIdInput = container.querySelector('#fadeRL-animationID')
  const delayBeforeStartInput = container.querySelector(
    '#fadeRL-delayBeforeStart'
  )
  const animationDelayInput = container.querySelector('#fadeRL-animationDalay')
  const speedInput = container.querySelector('#fadeRL-animationSpeed')
  const slowdownEffectCheckbox = container.querySelector(
    '#fadeRL-slowdownEffect'
  )
  const endSlowdownEffectCheckbox = container.querySelector(
    '#fadeRL-endSlowdownEffect'
  )
  const generateBtn = container.querySelector('#generate-fadeRL')
  const copyBtn = container.querySelector('#copy-fadeRL')
  const title = container.querySelector('#title')
  const output = container.querySelector('#fadeRL-output')
